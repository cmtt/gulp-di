...
module.exports = (function () {
  ...
  class GulpDI {
    ...
    constructor (gulp, options) {
      ...
      // Ensure loadPlugins beign called with the default patterns (see above)

      for (let i = 0, l = defaultPatterns.length; i < l; i++) {
        let pattern = defaultPatterns[i];
    ...
    parseOptions (options) {
      ...
      // Create a getter in order to defer accessing the plugin in this case.

      if (!!options.lazy) {
        let pluginKeys = propertyKeys(plugins);
        for (let i = 0, l = pluginKeys.length; i < l; i++) {
          let key = pluginKeys[i];
          this.providePlugin(key, plugins);
        }
      } else {
